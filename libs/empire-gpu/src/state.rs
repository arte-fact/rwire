...
/// seat in the batch's genomes, what each brain reads of its sight, the
/// rung each brain is read at, the table's own rung, its length and its
/// letters.
        ...
        Table {
            ...
            barbarians: game.barbarians_surface,
                ...
                game.year > seating.longest
                    || outcomes.iter().any(|o| o.crowned.is_some())
                    || outcomes.iter().all(|o| o.fell.is_some()),
            ),
            rng: rng.words(),
            genome: seating.genomes,
            reads: seating.reads.map(Reads::bits),
