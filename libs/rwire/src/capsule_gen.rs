...
let f=b.k&32?n=>n.toLocaleString(document.documentElement.lang||undefined):String;if(k===1)e.style.width=Math.min(100,Math.max(0,p*100))+'%';else if(k===2)e.textContent=f(b.d?Math.round(v*b.n/b.d):0);else if(k===3){let i=0;while(i<b.t.length&&v>=b.t[i])i++;for(let j=0;j<e.children.length;j++)e.children[j].hidden=j!==i}else if(k===4){let q=Math.min(1,Math.max(0,p))*(b.t.length-1),j=Math.floor(q),y=Math.round(b.o+b.t[j]+(b.t[Math.min(j+1,b.t.length-1)]-b.t[j])*(q-j));e.textContent=b.k&64?(y>0?'+':y<0?'\u2212':'')+f(Math.abs(y)):f(y)}else e.textContent=f(v)}}
const A={4:'id'};
