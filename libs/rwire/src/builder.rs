...
impl ElementBuilder {
    ...
    /// Feed a live value channel from this input: on every `input` event the
    /// client pushes the value to the elements bound with [`Self::live_text`] /
    /// [`Self::live_fill`] — no server round-trip. Channels come from
    /// [`next_live_channel`].
    pub fn live_source(mut self, channel: u16) -> Self {
        self.live_source = Some(channel);
        self
    }
    ...
    fn hash_content(&self, hasher: &mut impl Hasher) {
        ...
        self.live_source.hash(hasher);
        self.live_binds.hash(hasher);
        for child in &self.children {
...
impl BuildContext {
    ...
    fn emit_client_action_bindings(&mut self, ref_idx: u32, el: &ElementBuilder) {
        emit_live_bindings(&mut self.buf, ref_idx, el);
        for tb in &el.target_bindings {
...
/// Emit live value source/binding opcodes for an element (client-side only, so
/// identical on initial render and updates).
fn emit_live_bindings(buf: &mut OpcodeBuffer, ref_idx: u32, el: &ElementBuilder) {
...
static NEXT_LIVE_CHANNEL: AtomicU32 = AtomicU32::new(1);

/// Allocate a fresh live value channel (see [`ElementBuilder::live_source`]).
pub fn next_live_channel() -> u16 {
    (NEXT_LIVE_CHANNEL.fetch_add(1, Ordering::Relaxed) % 0x3FFF) as u16 + 1
}
...
fn emit_update_element(
    ...
    emit_live_bindings(buf, ref_idx, el);
