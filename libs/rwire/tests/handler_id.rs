...
use rwire::{State, handler, stable_handler_id};
...
fn macro_assigns_stable_nonzero_id() {
    ...
    assert_eq!(a, b, "the same handler must yield the same id across calls/renders");
    assert_ne!(inc().handler_id(), dec().handler_id(), "distinct handlers need distinct ids");
}
...
fn handler_ids_survive_the_wire_varint() {
    ...
    let check = |id: u32| {
        ...
        assert_eq!(read_varint(&buf), Some((id, buf.len())), "id {id} must round-trip");
    };
    check(inc().handler_id());
    check(dec().handler_id());
    for (m, n) in [("a::b::c", "do_thing"), ("x", "y"), ("very::long::module::path", "handler")] {
