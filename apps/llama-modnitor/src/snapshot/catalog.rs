...
/// llama.cpp flags — unsloth-recommended values first (Gemma 4 / Qwen3.6).
const LLAMA_CPP_FLAGS: &[FlagSpec] = &[
...
/// vLLM flags, per `vllm serve --help`.
const VLLM_FLAGS: &[FlagSpec] = &[
    FlagSpec {
...
/// flambeau flags, per the binary's `--help`.
const FLAMBEAU_FLAGS: &[FlagSpec] = &[
    FlagSpec {
        flag: "--mesh-mode",
...
pub const fn flag_catalog(engine: EngineKind) -> &'static [FlagSpec] {
    match engine {
        EngineKind::LlamaCpp => LLAMA_CPP_FLAGS,
        EngineKind::Vllm => VLLM_FLAGS,
        EngineKind::Flambeau => FLAMBEAU_FLAGS,
    }
}
