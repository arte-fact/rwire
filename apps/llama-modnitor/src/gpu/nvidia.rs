...
/// Reads NVIDIA metrics via `nvidia-smi --query-gpu`.
...
/// Parse the comma-separated `nvidia-smi` query output into per-card metrics.
pub fn parse_nvidia_csv(csv: &str) -> BTreeMap<String, GpuMetrics> {
    ...
    for line in csv.lines() {
        ...
        let power_limit = convert::f64_u32(fields[5].parse::<f64>().unwrap_or(0.0));
...
mod tests {
    ...
    fn test_parse_nvidia_csv() {
        ...
        let metrics = parse_nvidia_csv(csv);
