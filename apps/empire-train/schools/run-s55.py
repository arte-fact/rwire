#!/usr/bin/env python3
"""s55 (septembre 2026) : s54 avec une mémoire apprise, et rien d'autre.

Le protocole de s54 tel quel — essais de zéro, `--hall 0`, `--longest 100`,
gardés à ≥ 1 % de sacres à la gén. VERDICT_GEN, les cinq autres chaises tirées
parmi les sept écoles livrées, verdict sur le bilan seule contre la prod
(sacres ≥ CROWNED %, médiane ≤ MEDIAN, chutes ≤ FELL %) — mais le cerveau
dispose désormais de `brain::RECALL` (32) sorties de plus à l'Extérieur, relues
l'année suivante par les deux réseaux (`Memory::last_orders`) : une mémoire
qu'il est libre de remplir, rien ne dit quoi retenir. En échange on lui retire
la mémoire toute faite (`Brain::told` faux) : plus de Chronique de l'an passé,
plus d'ordres de l'an passé, les rapports et lettres ne se lisent que l'année
où ils arrivent. Il garde ce qui est sous ses yeux : les chiffres du royaume,
la place, les rumeurs de la campagne qui vient de se jouer. Les sept écoles de
prod restent racontées (`told`), sans recall : elles jouent comme avant.

s54 est le témoin : 91 essais, 46 perdus, 38 finalistes au plateau (sacres
9–26,5 %, médiane an 30–91, chutes 73,5–91 %), un VALID (s54a : 68 % / an 22 /
32 %). On compare la distribution des finalistes et le taux de VALID.
Journal dans s55.log, bilan de chaque finaliste à la fin de son s55-…log.
"""
...
def keep(t, name):
    """Files the trial away as `s55{name}` (a letter for a valid school, `-finalist{n}` for
    one that entered the race but finished off the mark)."""
    for src, dst in (
        (t["log"], f"s55{name}.log"),
        (t["out"], f"s55{name}-war.json"),
        (t["out"].replace(".json", ".best.json"), f"s55{name}-war.best.json"),
    ):
        shutil.move(src, dst)
    with open(f"s55{name}.log", "a") as f:
        f.write(f"SCHOOL 55{name} DONE\n")


def main():
    os.chdir(HERE)
    subprocess.run(["cargo", "build", "--release", "-p", "empire-train"], check=True)
    valid = [c for c in LETTERS if os.path.exists(f"s55{c}-war.json")]
    ...
    while todo:
        ...
        for t in list(running):
            ...
            if b[0] < CROWNED or b[1] > MEDIAN or b[2] > FELL:
                ...
                note(f"try {t['n']}: off the mark (bilan: {told(b)}), kept as s55-finalist{t['n']}")
                continue
            letter = todo.pop(0)
            keep(t, letter)
            note(f"try {t['n']}: VALID → s55{letter} (bilan: {told(b)})")
    for t in running:
