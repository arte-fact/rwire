...
#[handler]
fn increment(s: &mut CounterState) {
    s.counter += 1;
}
#[handler]
fn decrement(s: &mut CounterState) {
    s.counter -= 1;
}
#[handler]
fn set_alice(s: &mut NameState) {
    s.name = "Alice".into();
}
#[handler]
fn set_bob(s: &mut NameState) {
    s.name = "Bob".into();
}
#[handler]
fn toggle(s: &mut EnabledState) {
    s.enabled = !s.enabled;
}
#[handler]
fn add_item(s: &mut ItemsState) {
    s.items.push(format!("Item {}", s.items.len() + 1));
}
#[handler]
fn remove_item(s: &mut ItemsState) {
    s.items.pop();
}
        ...
        .child(
            Stack::column()
                ...
                .children([
                    ...
                    Text::body("Each handler only re-renders its own state type.")
                        .muted()
                        .build(),
...
fn section(
    ...
    controls: ElementBuilder,
    renderer: ElementBuilder,
) -> ElementBuilder {
    Card::new()
        .child(
            Stack::column()
...
fn render_enabled(s: &EnabledState) -> ElementBuilder {
    ...
    let (status, intent) = if s.enabled {
        ("ON", BadgeIntent::Success)
    } else {
        ("OFF", BadgeIntent::Error)
    };
    Stack::row()
        .gap(Gap::Sm)
        .align_center()
