...
import { ls, lv, type LiveEl } from "./live.ts";
...
export function me(a: Node, b: Node): void {
  ...
  for (let k = 0; k < ba.length; k++) {
    ...
    if (ae.getAttribute(n) !== ba[k].value) {
      ae.setAttribute(n, ba[k].value);
      // Once the user has touched a control, setAttribute alone no longer
      // moves it (the dirty flag): a re-rendered slider would keep its old
      // position. The focused control still gets its typed value back at
      // BATCH_END.
      if (n === "value" && "value" in ae) (ae as any).value = ba[k].value;
    }
  }
  ...
  ae.__k = be.__k;
  ae.__b = be.__b;
  // Live bindings and sources move onto the reused node (live.ts).
  const al = ae as LiveEl,
    bl = be as LiveEl;
  if (al.__lb) for (const x of al.__lb) x.e = null;
  if ((al.__lb = bl.__lb)) for (const x of al.__lb) x.e = al;
  if (al.__lc !== undefined && al.__lc !== bl.__lc && lv[al.__lc] === al) delete lv[al.__lc];
  if (bl.__lc !== undefined) ls(al, bl.__lc);
  else al.__lc = undefined;
