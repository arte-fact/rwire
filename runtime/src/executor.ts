...
import { ls, ul, lv, lb, type LiveBind, type LiveEl, type LiveSum } from "./live.ts";
...
export function x(d: Uint8Array): void {
  ...
  try {
    while (i < d.length) {
      ...
      if (o === OP.SYMBOLS) {
      ...
      } else if (o === OP.LIVE_SOURCE) {
        const [f, fl] = rv(d, i);
        i += fl;
        const [c, cl] = rv(d, i);
        i += cl;
        ls(r[f], c);
      } else if (o === OP.LIVE_BIND) {
        // [ref, channel, kind, args…] — see protocol/opcodes.rs LIVE_BIND.
        const [f, fl] = rv(d, i);
        i += fl;
        const [c, cl] = rv(d, i);
        i += cl;
        const k = d[i++];
        const b: LiveBind = { e: r[f], k };
        let n: number, nl: number;
        if (k & 16) {
          [b.b, nl] = rv(d, i);
          i += nl;
          n = d[i++];
        ...
        if (kind === 2) {
          ...
          if (k & 128) b.h = rt();
          if (kind === 6)
            for (const q of ["a0", "a1", "w0", "w1"] as const) {
              const [x2, xl] = rv(d, i);
              i += xl;
              b[q] = zz(x2);
            }
          b.c = [...new Set(b.l.s.concat(b.h ? b.h.s : []).map((x) => x.c))];
        }
        const le = r[f] as LiveEl;
        (le.__lb || (le.__lb = [])).push(b);
        for (const h of b.c || [c]) (lb[h] || (lb[h] = [])).push(b);
      } else if (o === OP.BATCH_END) {
        fm();
        // A scroll box marked data-autoscroll follows its bottom (journals, logs).
        document.querySelectorAll("[data-autoscroll]").forEach((e) => {
        ...
        // Live bindings: forget what left the document, re-run every channel
        // so a re-render never leaves a readout stale or detached.
        for (const c in lb) {
          lb[c] = lb[c].filter((x) => x.e && x.e.isConnected);
          if (!lb[c].length) delete lb[c];
        }
        for (const c in lv) {
          if (lv[c].isConnected) ul(+c);
          else delete lv[c];
        }
        if (st.onBatch) st.onBatch();
        return;
      } else {
        console.error(
          "Unknown opcode 0x" +
