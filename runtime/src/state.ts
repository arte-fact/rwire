...
export type RwEl = HTMLElement & {
  ...
  /** Live value channel this node is the source of, and its listener mark (live.ts). */
  __lc?: number;
  __ls?: 1;
  /** Live value bindings on this node (live.ts). */
  __lb?: import("./live.ts").LiveBind[];
};
...
export const st = {
  ...
  /** Told at every BATCH_END: a batch applied end to end (connect.ts resets its backoff on it). */
  onBatch: null as (() => void) | null,
};
