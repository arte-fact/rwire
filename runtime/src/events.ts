...
export function gp(e: Event, el: RwEl): string {
  ...
  if (e.type === "input" || e.type === "change") {
    // A binding on a wrapper (a labelled field, a slider's frame) reports the
    // control that fired inside it.
    const tg = e.target as Element | null;
    const c = (tg && /^(INPUT|TEXTAREA|SELECT)$/.test(tg.tagName) ? tg : el) as HTMLInputElement;
    if (/^(INPUT|TEXTAREA|SELECT)$/.test(c.tagName))
      return JSON.stringify({ t: "text", v: c.value });
  }
  if (e.type === "click") {
