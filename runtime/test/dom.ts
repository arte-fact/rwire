...
export function makeDom(): { document: MockDoc } {
  ...
  // Every node ever given an id: getElementById answers with the one in the
  // document, as a browser does — a shadow subtree built for a morph carries
  // the same ids as the live one it replaces (nested regions), and must never
  // shadow it.
  const allById = new Map<string, MockEl[]>();
  const register = (id: string, n: MockEl) => {
    byId.set(id, n);
    (allById.get(id) || allById.set(id, []).get(id)!).push(n);
  };
  ...
  function el(tag: string): MockEl {
    ...
    const node: MockEl = {
      ...
      setAttribute(n, v) {
        ...
        if (n === "id") register(String(v), node);
      },
      ...
      querySelectorAll(sel) {
        ...
        const hits: MockEl[] = [];
        if (byIdSel === null && attrSel === null && tagSel === null) return hits;
        const walk = (e: MockEl): void => {
          if (
            (byIdSel !== null && e.id === byIdSel) ||
            (tagSel !== null && e.tagName === tagSel && e !== node) ||
            (attrSel !== null &&
  ...
  const doc: MockDoc = {
    ...
    getElementById: (id) => {
      const nodes = allById.get(id) || [];
      return nodes.find((n) => n.isConnected) || byId.get(id) || null;
    },
    querySelector: (sel: string) => doc.body.querySelector(sel),
    querySelectorAll: (sel: string) => doc.body.querySelectorAll(sel),
