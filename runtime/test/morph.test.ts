...
import { lv, lb, resetLive, type LiveBind } from "../src/live.ts";
...
test("me pushes a changed value attribute onto the live property", () => {
  const a = doc.createElement("input"),
    b = doc.createElement("input");
  a.setAttribute("value", "3");
  a.value = "8"; // the user moved it: setAttribute alone would no longer move the control
  b.setAttribute("value", "5");
  me(a as any, b as any);
  assert.equal(a.getAttribute("value"), "5");
  assert.equal(a.value, "5");
});

test("me carries live bindings and the source registration onto the reused node", () => {
  const a = doc.createElement("span") as any,
    b = doc.createElement("span") as any;
  const stale: LiveBind = { e: a, k: 0 },
    fresh: LiveBind = { e: b, k: 0 };
  a.__lb = [stale];
  b.__lb = [fresh];
  a.__lc = 1;
  lv[1] = a;
  ...
  lb[2] = [fresh];
  me(a, b);
  assert.equal(stale.e, null, "the old binding is detached");
  assert.equal(fresh.e, a, "the new binding points at the live node");
  assert.equal(a.__lb, b.__lb);
  assert.equal(lv[1], undefined, "the old channel forgets the node");
  assert.equal(lv[2], a, "the node is the source of the new channel");
  assert.equal(a.__lc, 2);
});
