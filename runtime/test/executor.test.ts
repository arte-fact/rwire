...
import { resetLive, lv, lb } from "../src/live.ts";
...
// --- live value bindings (LIVE_SOURCE 0x51 / LIVE_BIND 0x52) ---

test("LIVE_SOURCE + LIVE_BIND: an input feeds text, fill and scaled readouts with no round-trip", () => {
  run(
    mapDef([[0, 1, "input"], [0, 2, "span"], [0, 3, "div"]]),
    [0x02, 1], // r0 = <input>
    [0x02, 2], // r1 = text readout
    [0x02, 3], // r2 = fill
    [0x02, 2], // r3 = scaled (v * 3 / 2)
    [0x20, ...vint(0xffff), ...vint(0)],
    [0x20, ...vint(0xffff), ...vint(1)],
    [0x20, ...vint(0xffff), ...vint(2)],
    [0x20, ...vint(0xffff), ...vint(3)],
    [0x51, ...vint(0), ...vint(7)], // source on channel 7
    [0x52, ...vint(1), ...vint(7), 0], // kind 0: text
    [0x52, ...vint(2), ...vint(7), 1], // kind 1: fill
    [0x52, ...vint(3), ...vint(7), 2, ...vint(3), ...vint(2)], // kind 2: scaled 3/2
  );
  const [input, text, fill, scaled] = doc.body.children as any[];
  assert.equal(lv[7], input, "the source is registered on its channel");
  assert.equal(lb[7].length, 3);
  assert.equal(text.textContent, "0", "BATCH_END runs every channel once");
  input.value = "25";
  input.min = "0";
  input.max = "50";
  input.fire("input");
  assert.equal(text.textContent, "25");
  assert.equal(fill.style.width, "50%");
  assert.equal(scaled.textContent, "38");
});

test("LIVE_BIND lookup interpolates a server table over the source's range, signed", () => {
  run(
    mapDef([[0, 1, "input"], [0, 2, "span"]]),
    [0x02, 1],
    [0x02, 2],
    [0x20, ...vint(0xffff), ...vint(0)],
    [0x20, ...vint(0xffff), ...vint(1)],
    [0x51, ...vint(0), ...vint(2)],
    // kind 4 | signed (0x40): 3 entries, lo = -10 (zigzag 19), offsets 0, 10, 30 → -10, 0, 20
    [0x52, ...vint(1), ...vint(2), 0x44, 3, ...vint(19), ...vint(0), ...vint(10), ...vint(30)],
  );
  const [input, out] = doc.body.children as any[];
  input.min = "0";
  input.max = "100";
  input.value = "0";
  input.fire("input");
  assert.equal(out.textContent, "\u221210");
  input.value = "75";
  input.fire("input");
  assert.equal(out.textContent, "+10");
});

test("BATCH_END scrolls data-autoscroll boxes to the bottom and prunes detached bindings", () => {
  const box = doc.createElement("div") as any;
  box.setAttribute("data-autoscroll", "1");
  ...
  run(
    mapDef([[0, 1, "input"], [0, 2, "span"]]),
    [0x02, 1],
    [0x02, 2], // r1 is never appended: its binding must not survive BATCH_END
    [0x20, ...vint(0xffff), ...vint(0)],
    [0x51, ...vint(0), ...vint(4)],
    [0x52, ...vint(1), ...vint(4), 0],
  );
  assert.equal(box.scrollTop, 480);
  assert.equal(lb[4], undefined, "a binding whose node is not in the document is forgotten");
  assert.ok(lv[4], "the connected source stays");
});

test("BATCH_END tells st.onBatch (the connection's backoff reset)", () => {
  let told = 0;
  st.onBatch = () => told++;
  run();
  assert.equal(told, 1);
