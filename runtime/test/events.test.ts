...
test("gp on input reports the control that fired inside a bound wrapper", () => {
  const label = doc.createElement("label");
  const input = doc.createElement("input");
  input.value = "hello";
  label.appendChild(input);
  const p = gp(fakeEv("input", input) as any, label as any);
  assert.deepEqual(JSON.parse(p), { t: "text", v: "hello" });
  // A wrapper without a control inside sends nothing.
  const div = doc.createElement("div");
  assert.equal(gp(fakeEv("input", div) as any, div as any), "");
});
