...
test("attribute names keep their case (SVG viewBox is case-sensitive)", () => {
  const e = el();
  sa(e as any, "viewBox", "0 0 24 24");
  assert.equal(e.getAttribute("viewBox"), "0 0 24 24");
  assert.equal(e.getAttribute("viewbox"), null);
});
